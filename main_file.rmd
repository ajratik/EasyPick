...
Решается задача предсказания пика в игре Dota 2. Проект для игроков, которые хотят подобрать оптимальных героев, с учетом уже выбранных и запрещенных героев. Общая логика приложения: пользователь указывает уже 4-х выбранных героев за каждую команду и приложение рекомендует 10 наиболее подходящих героев с их вероятностью победы.

В процессе работы были построены следующие модели: Catboost, Random Forest с ядром ranger, логистическая регрессия, дерево решений с кроссвалидацией на C5.0) и градиентый бустинг XGBoost. В финальной версии использовался Catboost. 

Нет сценария использования для разных типов пользователей, будем добавлять это в следующих версиях(для этого достаточно сделать две одинаковые
модели на данных матчей опытных и неопытных игроков).
...
data = data %>% 
  select(radiant_win:dire_pick_5)
...
#splitting data
set.seed(14)
split_data <- initial_split(data, prop = 8/10)

test_data=testing(split_data)
train_data=training(split_data)

#selecting predictors in test and train for champ_recipe
champ_recipe = train_data %>%
  recipe(radiant_win ~ dire_pick_1 + dire_pick_2+ dire_pick_3 + dire_pick_4 + dire_pick_5 + radiant_pick_1 + radiant_pick_2 + radiant_pick_3 + radiant_pick_4 + radiant_pick_5) %>%
  prep()

test_prep = champ_recipe %>%
  bake(test_data) 
train_prep = juice(champ_recipe)

#random forest model
set.seed(14)
md_rf <-  rand_forest(trees = 1000, mode = "classification",mtry=8) %>%
  set_engine("ranger") %>%
  fit(radiant_win ~ dire_pick_1 + dire_pick_2+ dire_pick_3 + dire_pick_4 + dire_pick_5 + radiant_pick_1 + radiant_pick_2 + radiant_pick_3 + radiant_pick_4 + radiant_pick_5, data = train_prep)

#evaluation rf
md_rf %>%
  predict(test_prep) %>%
  bind_cols(test_prep)  %>% 
  metrics(truth = radiant_win, estimate = .pred_class)

#summary rf
summary(md_rf$fit$forest)
...
#wf
rf_rec <- recipe(radiant_win~. , data = train_data)
tune_wf <- workflow() %>%
  add_recipe(rf_rec) %>%
  add_model(tune_rf)
...
tree_rec <- recipe(radiant_win ~ ., data = train_data) 
...
#CV
set.seed(14)
vb_folds <- vfold_cv(train_data, strata = radiant_win)
...
Как оказалось, (на удивление) Кэтбуст очень не дружит с R. В ходе работы появился баг (на вход для работы нужна одна категория данных, на выходе другая). Решилась эта проблема с помощью Гитхаба. Пользователь вручную видоизменил код модели (в один момент вручную поменял тип переменных). 
...
Для удобства, мы отдельно скачали модель, с уже вставленными параметрами
```{r}
load("model.RData")
```
...
Данные для обучения берутся при помощи парсера, он получает данные с opendota api. У него есть различные конфигурации:
https://github.com/Chinaskihate/EasyPick/blob/f/f-backend/src/backend/Backend/MatchParser.Service/appsettings.json

Настройки загрузки и настройки валидаций для матчей.

Путь данных от клиента до клиента:
1)Вводит выбор в UI
2)WebApi получает данные и пробрасывает их в очередь
3)RequestRedirector считывает из очереди и сохраняет новые матчи себе в память
4)Модель периодически достает новые запросы из RequestRedirector-а
5)Модель прогнозирует и отправляет запрос в RequestRedirector
6)RequestRedirector пробрасывает в очередь
6)WebApi считывает из очереди и готовит ответ
7)UI периодически опрашивает WebApi по Id запроса, отображает по готовности

Поскольку наша модель заранее установлена и используется в самом приложении, то никаких изменений с исходными данными не происходит, кроме тех, что нужны непосредственно в процессе работы приложения. Данные из приложения мы никуда не сохраняем.
...
**Скрин интерфейса:** 
![](C:\Users\letsp\Downloads\скрин7.jpg) 
![](C:\Users\letsp\Downloads\скрин8.jpg) 
...
Первые колонка шансы на победу рэдиант, вторая колонка герой для radiant команды, третья - шансы на победу даер, четвертая - герой для dire
...
![](C:\Users\letsp\Downloads\скрин1.jpg)
...
![](C:\Users\letsp\Downloads\скрин2.jpg)
...
![](C:\Users\letsp\Downloads\скрин3.jpg)
...
![](C:\Users\letsp\Downloads\скрин4.jpg)
...
![](C:\Users\letsp\Downloads\скрин5.jpg)
...
![](C:\Users\letsp\Downloads\скрин6.jpg)
